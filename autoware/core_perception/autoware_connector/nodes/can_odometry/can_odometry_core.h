...
struct Odometry
{
  ...
  void updateOdometry(const double vx, const double vth, const ros::Time& cur_time)
  {
    ...
    double dt = (cur_time - stamp).toSec();　//获取当前时间与上一时刻的时间间隔
    double delta_x = (vx * cos(th)) * dt; //计算单位时间段ｘ方向运动过的距离  简单理解就是s=v*t
    double delta_y = (vx * sin(th)) * dt; //同上
    double delta_th = vth * dt; //计算得出角度
